...
var $author$project$Main$init = function (_v0) {
	var seeded = $author$project$Main$withInitialPositions($author$project$Main$initialPropositions);
...
var $author$project$Main$toggleCard = F2(
	function (propositionId, model) {
		return model.suppressNextOpen ? _Utils_Tuple2(
			...
			_Utils_update(
				model,
				{
					expandedPropositionId: _Utils_eq(
						model.expandedPropositionId,
						$elm$core$Maybe$Just(propositionId)) ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just(propositionId),
	...
	function (propositionId, model) {
		...
		if (_v0.$ === 'Just') {
			...
			return (!_Utils_eq(dragState.propositionId, propositionId)) ? _Utils_Tuple2(model, $elm$core$Platform$Cmd$none) : (dragState.moved ? _Utils_Tuple2(
				...
				$author$project$Main$toggleCard,
				propositionId,
				_Utils_update(
					model,
					{dragging: $elm$core$Maybe$Nothing})));
		} else {
			return A2($author$project$Main$toggleCard, propositionId, model);
		}
	});
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							...
							expandedPropositionId: _Utils_eq(
								model.expandedPropositionId,
								$elm$core$Maybe$Just(propositionId)) ? $elm$core$Maybe$Nothing : model.expandedPropositionId,
							selectedPropositionId: $elm$core$Maybe$Just(propositionId),
							suppressNextOpen: false
						}),
					A2(
...
var $author$project$Main$ToggleExpanded = function (a) {
	return {$: 'ToggleExpanded', a: a};
};
var $author$project$Main$expandedScale = 1;
...
var $author$project$Main$onExpandedTouchEnd = function (propositionId) {
	return A2(
		$elm$html$Html$Events$stopPropagationOn,
		'touchend',
		$elm$json$Json$Decode$succeed(
			_Utils_Tuple2(
				$author$project$Main$ToggleExpanded(propositionId),
				true)));
};
		...
		if (_v0.$ === 'Nothing') {
			...
			var interactionAttributes = isExpanded ? _List_fromArray(
				[
					...
					$author$project$Main$ToggleExpanded(item.id)),
					$author$project$Main$onExpandedTouchEnd(item.id)
				]) : _List_fromArray(
				[
					A2(
					$elm$html$Html$Events$preventDefaultOn,
			...
			var cardScale = isExpanded ? $author$project$Main$expandedScale : $author$project$Main$miniScale;
			var scaledHeight = $author$project$Main$miniatureHeight * cardScale;
			var scaledWidth = $author$project$Main$miniatureWidth * cardScale;
			return A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$style,
						'z-index',
						isDragging ? '80' : (isExpanded ? '70' : '30'))
					]),
				_List_fromArray(
					[
						A2(
						...
						_Utils_ap(
							...
							_List_fromArray(
								[
									...
									$elm$html$Html$Attributes$style,
									'transform',
									'scale(' + ($elm$core$String$fromFloat(cardScale) + ')')),
									...
									A2($elm$html$Html$Attributes$style, 'transition', 'transform 220ms cubic-bezier(0.22, 1, 0.36, 1)')
								])),
						_List_fromArray(
							[
