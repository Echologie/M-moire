...
const bank=JSON.parse(fs.readFileSync('research/bank.json')),publicBank=JSON.parse(fs.readFileSync('docs/site/data/bank.json')),codes=JSON.parse(fs.readFileSync('research/codebook.json'));
assert.equal(bank.questions.length,40);let ids=new Set(),formulas=0,counts={},levels={};const ctx={};vm.createContext(ctx);vm.runInContext(fs.readFileSync('docs/site/vendor/katex/katex.min.js','utf8'),ctx);
...
for(const f of ['index.html','enquete.html','enquete.css','enquete.js','data/bank.json','prototype.html'])assert.deepEqual(fs.readFileSync('docs/site/'+f),fs.readFileSync('docs/site/'+f),f);
for(const f of ['index.html','enquete.html'])for(const m of fs.readFileSync('docs/site/'+f,'utf8').matchAll(/(?:src|href)="([^"]+)"/g))assert.ok(fs.existsSync('docs/site/'+m[1]),m[1]);
